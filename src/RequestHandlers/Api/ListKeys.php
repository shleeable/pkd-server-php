...
class ListKeys implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $publicKeys = $this->publicKeysTable->getPublicKeysFor($actorID);
        if (empty($publicKeys)) {
            return $this->error('Actor not found or has no registered public keys', 404);
        }
        foreach ($publicKeys as $i => $pk) {
            if (isset($pk['public-key'])) {
                if ($pk['public-key'] instanceof PublicKey) {
                    $publicKeys[$i]['public-key'] = $pk['public-key']->toString();
                }
            }
        }
        return $this->json([
